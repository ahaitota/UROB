...
def backward(seed: int):
    ...
    try:
        ...
        # Random tensor data simulating logits and integer target labels
        logits = Tensor(np.random.rand(5, 4))  # 5 samples, 4 classes
        # print("logits")
        print(logits)
        target = np.random.randint(0, 4, size=5)  # random targets for 5 samples with 4 classes

        # Forward pass
        loss = logits.cross_entropy_loss(target)
        print("Forward pass - Loss:", loss.data)

        # Backward pass
        loss.backward()
        
        # Print gradients for inspection
        print("Gradients - logits:", logits.grad)
